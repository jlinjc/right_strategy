...
# ★2026-09-24 Jason:「我本來就有投資美股,不要 00757」→ 00757(統一FANG+=美股FANG+台幣計價)移出核心。
#   替代標的實測(research_b3_tw_replace.py,4檔基底+第5檔、同期比較、等權+信用哨):
#   00757 +0.094(p=0.165)、00692 +0.014、0056 +0.001、00733 −0.014、00762 −0.001、
#   00895 −0.016、00892 −0.041、00913 −0.056、00891 −0.060;00921 期間<3年不判定。
#   → 沒有任何候選達到「ΔSharpe≥0.10 且 p<0.10」→ 台股核心 = 4 檔,不放第5檔。
...
TW_WATCH = {
    ...
    # ★2026-09-23 Jason 指示接入:正2(槓桿ETF,只當「衛星」用,不進等權核心)。
    #   實證(research_q4_choices.py,2015-08~2026-09):核心80%+正2衛星20% Sharpe 1.21~1.28,
    #   比純核心 1.36 更差;想加報酬,核心等權×1.2(Sharpe 1.35/CAGR 25.2%)優於加正2衛星。
    #   純買持正2 CAGR 41.5% 但 MDD −55%、最差年 −36%;正2+200MA擇時反而更差(Sharpe 0.92)。
    #   → live 只提供資料與規則,配多少由 v2 頁面的決策清單自己選。
    '00631L.TW': '元大台灣50正2(槓桿)',
    '00757.TW': '統一FANG+(美股FANG+台幣計價;2026-09-24 移出核心,僅供參考)',
}
